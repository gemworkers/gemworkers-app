...
export function HeroSection({ hasHero = false }: { hasHero?: boolean }) {
  ...
  return (
    <section
      ...
      style={{ position: hasHero ? 'relative' : undefined }}
    >
      {/* Optional hero.jpg background */}
      {hasHero && (
        <>
          <div style={{ position: 'absolute', inset: 0, zIndex: 0, overflow: 'hidden' }}>
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img src="/hero.jpg" alt="" style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />
          </div>
          <div style={{
            position: 'absolute', inset: 0, zIndex: 0,
            background: 'linear-gradient(160deg, rgba(14,14,16,0.52) 0%, rgba(14,14,16,0.86) 100%)',
          }} />
        </>
      )}

      {/* All content sits above background layers */}
      <div style={{ position: hasHero ? 'relative' : undefined, zIndex: hasHero ? 1 : undefined }}>
        ...
        <h1 style={{
          ...
          fontWeight: 300, letterSpacing: '0.08em', color: '#f5f0e8',
          fontFamily: 'var(--font-cormorant, Georgia, serif)',
          lineHeight: 1.1, marginBottom: 32, textTransform: 'uppercase',
        }}>
          {WORDS.map((word, i) => (
            <span
              key={i}
        ...
        <div style={{ width: 56, margin: '0 auto 28px', position: 'relative', height: 1, overflow: 'hidden' }}>
        ...
        <p
          className="hero-sub"
          style={{
            fontSize: 'clamp(14px, 2vw, 17px)', color: '#9d9080',
        ...
        <Link
          ...
          style={{
            ...
            border: '1px solid rgba(201,169,98,0.6)', borderRadius: 4,
            fontSize: 12, fontWeight: 600, letterSpacing: '0.14em',
            textTransform: 'uppercase', color: '#c9a962',
