...
import type { EmailOrderLine, ShippingAddress } from './sendNewOrderEmail'
...
function buildSubject(orders: EmailOrderLine[]): string {
  if (orders.length === 1 && orders[0]) {
    return `Your GemWorkers order is confirmed — ${orders[0].itemTitle}`
  }
  return 'Your GemWorkers order is confirmed'
}
...
<body style="margin:0;padding:40px 24px;background:#fafaf9;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Helvetica,Arial,sans-serif;color:#111">
  <div style="max-width:520px;margin:0 auto">
    ...
    <h1 style="font-family:Georgia,'Times New Roman',serif;font-size:22px;font-weight:700;margin:0 0 8px;color:#111;line-height:1.2">Order confirmed</h1>
    <p style="font-size:14px;color:#6b7280;margin:0 0 28px;line-height:1.6">Thank you for your order. We&rsquo;ll let you know when it ships.</p>
...
export async function sendOrderConfirmationEmail(opts: {
  ...
  await getResend().emails.send({
    from:    FROM,
    to:      opts.to,
    subject: buildSubject(opts.orders),
